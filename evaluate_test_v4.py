"""
Test evaluator for v4 model — 7 classes with merging
Same VALUE_MAP as train_v4.py
"""
...
# Load v4 model
print("Loading SegFormer-B4 v4 model...")
model = SegformerForSemanticSegmentation.from_pretrained(
    "nvidia/segformer-b4-finetuned-ade-512-512",
    num_labels=NUM_CLASSES,
    ignore_mismatched_sizes=True)
model.load_state_dict(torch.load(
    os.path.join(script_dir, 'runs', 'best_segformer_b4_v4.pth'),
    map_location=device))
...
print("Evaluating on TEST SET with TTA (5 augmentations)...")
with torch.no_grad():
...
print(f"\n{'='*55}")
print(f"TEST SET — 7 Classes")
print(f"mIoU: {mIoU:.4f} | mAP: {mAP:.4f}")
print(f"{'='*55}")
...
plt.title(f'TEST SET Per-Class IoU — SegFormer-B4 v4\n'
          f'mIoU={mIoU:.4f} | mAP={mAP:.4f}')
plt.ylim(0, 1.0)
plt.legend()
plt.tight_layout()
plt.savefig('test_v4_per_class_iou.png', dpi=150)
plt.close()
...
plt.tight_layout()
plt.savefig('test_v4_confusion_matrix.png', dpi=150)
plt.close()

print(f"\nSaved: test_v4_per_class_iou.png")
print(f"Saved: test_v4_confusion_matrix.png")
